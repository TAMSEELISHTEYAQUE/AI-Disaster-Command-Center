function getMissionPhase(severity) {
    ...
    if (value.includes("critical") || value.includes("catastrophic")) return { phase: "Active", detail: "Top-priority response operations are underway across the impact zone.", teams: "9 teams deployed" };
    if (value.includes("high") || value.includes("severe")) return { phase: "En Route", detail: "Rapid-response assets are moving toward the affected districts.", teams: "6 teams deployed" };
    if (value.includes("moderate") || value.includes("medium")) return { phase: "Deploying", detail: "Support units are being staged for local mobilization.", teams: "4 teams deployed" };
    if (value.includes("low")) return { phase: "Completed", detail: "Monitoring remains active while the response footprint is reduced.", teams: "1 team standby" };
